...
untouched ("confirm ground reality", "don't change NR MCP wiring"). Record live path + what the
new feature must **not** replace. *Session evidence (spec 032): LLM NR tools preserved; server
link intake added in parallel.*
...
5. **Git / worktree layout** — when the design touches checkout, fetch, push, or publish on an
   **existing remote branch** (reuse, append, rebase, force-with-lease), open the worktree
   ...
   worktree is **REJECTED** until a real-git fixture or spike proves the incantation. *Session
   evidence (spec 027): DB reuse + mocked `_run_git` tests were green; live E2E failed on
   `origin/<branch> not found` and `mirror=true` fetch refusal until `fetch_origin_tracking_refs`
   was added.*
...
|---|----------|----------------------------------|
| 1 | Layer / boundary legality | Which layer does each new module live in? All imports legal? Which reuse claims were rejected as illegal, and what replaced them? **Parallel-capability variant:** when adding server-side automation adjacent to existing LLM/MCP/agent tools, document both paths in the ledger — preserved path, new path, overlap rule, and a test or grep guard that new code does not illegally import or replace the preserved path. *Session evidence (spec 032): server `nr_link_intake` vs LLM `newrelic_tools` — D10 rejects `agents.tools` in services.* |
